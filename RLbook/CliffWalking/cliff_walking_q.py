...
for j in range(runs):
	...
	for i in range(episodes):
		...
		while (pos_y>0 or pos_x==0):
			...
			if new_random_value >= epsilon:
				new_action_index = np.argmax(q[pos_x][pos_y])
			new_action = actions[action_index]
	
			q[pos_x][pos_y][action_index] = q[pos_x][pos_y][action_index] + alpha*(reward + gamma*q[new_pos_x][new_pos_y][new_action_index] - q[pos_x][pos_y][action_index])
			pos_x = new_pos_x
			pos_y = new_pos_y
...
with open("cliff_sarsa.txt", "wt") as f:
